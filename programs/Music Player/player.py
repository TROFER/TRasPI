...
from typing import Awaitable, IO, overload
import enum
import io
# import shutil
...
class Player:
    ...
    def __stream_data_callback(self, outdata: list, frames: int, time_info, status: sd.CallbackFlags):
        if status:
            core.log.traceback.debug("SoundPlayer Stream Status: %s", status)
        try:
            data = self.__buffer.get_nowait()
        except queue.Empty as e:
            core.log.traceback.debug("SoundPlayer Stream Queue is Empty. Increase Buffersize")
            return
    ...
    def __preload(self):
        if self.__tracks and not self.__tracks[0]._ready.is_set():
            core.Interface.schedule(self.__tracks[0]._preload(self.__stream.samplerate, self.__stream.channels, self.__DTYPE))
