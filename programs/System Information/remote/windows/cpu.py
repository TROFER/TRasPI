...
class Main(core.render.Window):
    ...
    def __init__(self, client):
        ...
        self.elements = [Line(Vector(0, 10), Vector(128, 10)),
                          Text(Vector(3, 5),
                               f"{client.SERVER_ADDR} - CPU", justify='L'),
    ...
    def refresh(self):
        ...
        self.elements[2].text = f"CPU Load: {self.current['CPU']['load']}%"
        self.elements[3].text = f"CPU Temp: {self.current['CPU']['temperature']}°C"
        self.elements[4].text = f"CPU Speed: {self.current['CPU']['speed']}"
        self.elements[7].pos2 = Vector(
            constrain(float(self.current["CPU"]["load"]), 1, 100, 3, 125), 48)
