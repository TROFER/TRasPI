...
class Main(core.render.Window):
    ...
    def refresh(self):
        ...
        self.elements[2].text = f"GPU Load: {self.current['GPU']['load']}%"
        self.elements[3].text = f"GPU Temp: {self.current['GPU']['temperature']}°C"
        self.elements[4].text = f"GPU Speed: {self.current['GPU']['speed']}Mhz"
        self.elements[7].pos2 = Vector(
            constrain(float(self.current["GPU"]["load"]), 1, 100, 3, 125), 48)
